...
async def test_deriv_connection() -> bool:
    logging.info("🔄 Initializing Deriv server connection test...")

    try:
        async with websockets.connect(
            settings.DERIV_API_URL, ping_interval=None, ping_timeout=None
        ) as ws:
            ...
            auth_request = json.dumps(
                {"authorize": settings.DERIV_API_TOKEN, "req_id": 1}
            )
            ...
            if "error" in auth_data:
                logging.error(
                    f"❌ API Authorization failed: {auth_data['error']['message']}"
                )
                return False
...
def train_linear_model(candles):
    ...
    try:
        ...
        df.to_csv(
            ...
            mode="a",
            index=False,
            header=not os.path.exists(settings.DATA_PATH),
        )
...
async def send_signal(symbol, signal, entry, tp, sl):
    message = (
        ...
        f"🎯 Asset: {settings.symbol_names[symbol]}\n"
        f"📈 Signal: {signal}\n"
        f"💰 Entry: {entry:.2f}\n"
        f"🎯 Take Profit: {tp:.2f}\n"
        f"🛑 Stop Loss: {sl:.2f}\n\n"
...
async def main():
    ...
    while True:
        try:
            for symbol in settings.symbol_names.keys():
                ...
                if (
                    all(x is not None for x in [signal_m5, signal_m15, signal_h1])
                    and signal_m5 == signal_m15 == signal_h1
                ):
                    logging.info(
                        f"🎯 Signal confirmation found for {symbol}: {signal_m5}"
                    )
